  ...
  return (
    <div style={{ position: 'relative', height: 44, margin: '8px 0', userSelect: 'none' }}>
      ...
      <input type="range" min={MIN_RANGE} max={MAX_RANGE} step={STEP} value={minVal}
        onChange={e => { const v = Number(e.target.value); if (v < maxVal - STEP) onMinChange(v); }}
      ...
      <input type="range" min={MIN_RANGE} max={MAX_RANGE} step={STEP} value={maxVal}
        onChange={e => { const v = Number(e.target.value); if (v > minVal + STEP) onMaxChange(v); }}
...
export default function BudgetStep({ data, update, onNext, onBack, language = 'en' }) {
  ...
  return (
    <div style={{ maxWidth: 580, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13, color: '#94A3B8', marginBottom: 6 }}>{p.rangeLabel}</div>
      ...
      <div>
        <div style={{ fontSize: 13, fontWeight: 600, color: NAVY, marginBottom: 12 }}>{p.quickSelect}</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {PRESETS.map(preset => {
            const active = data.budgetMin === preset.min && data.budgetMax === preset.max;
            return (
              <button key={preset.label} onClick={() => update({ budgetMin: preset.min, budgetMax: preset.max })} style={{
                padding: '8px 16px',
